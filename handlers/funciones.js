...
const suerteSchema = require(`${process.cwd()}/modelos/suerte.js`);
...
async function asegurar_todo(guildid, userid) {
    ...
    if (userid) {
        let luckdata = await suerteSchema.findOne({ userID: userid })
        if (!luckdata) {
            console.log(`Asegurado: Suerte de ${userid}`.green);
            luckdata = await new suerteSchema({
                userID: userid
            });
            await luckdata.save();
        }
